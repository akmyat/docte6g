...
#include "ns3/ns3sionna-cfr-tag.h"
 ...
 *
 *  To run: ./example-ns3sionna-sensing-mobile.sh
 */
...
// CSI
std::string csiFname = "example-ns3sionna-sensing-mobile.csv";
std::ofstream ofs_csi(csiFname);
// Pathloss
std::string plFname = "example-ns3sionna-sensing-mobile-pathloss.csv";
std::ofstream ofs_pl(plFname);
// RX (STA) node location
std::string tpFname = "example-ns3sionna-sensing-mobile-time-pos.csv";
...
main(int argc, char* argv[])
{
    ...
    if (tracing)
    {
        ...
        spectrumPhy.EnablePcap("example-ns3sionna-sensing", apDevices.Get(0));
    }
