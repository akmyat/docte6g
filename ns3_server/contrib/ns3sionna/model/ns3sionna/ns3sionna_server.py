    ...
    def compute_cfr_with_lookahead(self, csi_req, reply_wrapper):
        ...
        for lah_time_idx, lah_time in enumerate(lah_time_vec): # iterate over time
            ...
            for curr_rx_id, curr_rx_node in enumerate(rx_nodes):
                ...
                lnk_tau_valid = lnk_tau[lnk_tau >= 0]
                if lnk_tau_valid.size > 0:
                    lnk_delay = int(round(np.min(lnk_tau_valid * 1e9), 0))
                else:
                    lnk_delay = 0
                ...
                if h_power > 0:
                    lnk_loss = float(-10 * np.log10(h_power))
                else:
                    lnk_loss = 400.0 # Extreme path loss for no paths found

                # for frequency-selective channel
                power = h_power  # use already calculated h_power
                if power > 0:
                    h_normalized = h / np.sqrt(power)
                else:
                    h_normalized = h # already zero
    ...
    def _walk(self, node_id, dt):
        ...
        while bounces < max_bounces:
            ...
            if si.is_valid():
                ...
                if t < 0:
                    # Corner/Wall stuck case: node is too close to a wall.
                    # Move exactly to the wall and stop there for this step.
                    pos = pos + t_raw * direction
                    distance = 0
                    dt = 0
                    break
