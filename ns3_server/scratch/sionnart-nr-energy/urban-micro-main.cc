...
NS_LOG_COMPONENT_DEFINE("SionnartNrUrbanMicro");
...
main(int argc, char* argv[]) {
    ...
    std::string scenario = "urban_micro";
    ...
    uint32_t seed = 42;
    // FR2 small-cell BS conducted power (TS 38.104 medium-range BS class).
    double gnbTxPowerDbm = 33.0;
    ...
    uint32_t numGnb = 1;
    uint32_t numUes = 30;
    // Defaults sized for 28 GHz mmWave (FR2). At this carrier, UMi street-canyon
    // reach is ~50 m with a 2x2 array (worst case in the --all sweep) and
    // extends to ~80-100 m at 8x8 via beamforming. Keep the radius conservative
    // so the smallest array still produces non-zero delivery for most UEs.
    double ueDist_min = 10.0;
    double ueDist_max = 80.0;
    // 3-sector site model (TR 38.901): each sector covers 120°, with ~60° HPBW.
    ...
    // DL or UL per slot based on actual queue demand. Empirically yields ~5x
    // better UL+DL throughput than semi-static DDDSU in this multi-UE setup
    // (semi-static DDDSU starves UL when 12+ UEs share 1-of-5 UL slots).
    // Set --tddPattern=DL|DL|DL|F|UL on the CLI to opt into commercial DDDSU.
    ...
    cmd.AddValue("scenario", "Fixed scene for this executable: urban_micro", scenario);
    ...
    if (all)
    {
        ...
        std::cout << "Running all urban-micro simulations: " << results.capacity()
        ...
        const std::filesystem::path summaryPath =
            std::filesystem::path(outputDir) / "urban_micro_all_runs_summary.csv";
    ...
    if (scenario != "urban_micro") {
        NS_FATAL_ERROR("This executable is urban-micro only. Use --scenario=urban_micro or omit the scenario argument.");
    }
    ...
    if (scenario == "urban_micro")
    {
        ...
        gnbHeight = 10.0;     // 3GPP UMi street-canyon base-station height
        if (ueDist_min <= 0.0) ueDist_min = 10.0;
        if (ueDist_max <= 0.0) ueDist_max = 80.0;    // 28 GHz UMi: street-canyon reach
    }
    else
    {
        NS_FATAL_ERROR("This executable is urban-micro only.");
    }
    ...
    // -----------------------------------------------------------------------
    if (scenario == "urban_micro") {
        if (f_c == 0.0) f_c = 28.0e9; // FR2 n257/n261-style UMi street-canyon carrier
        if (scs == 0) scs = 120000; // 120 kHz FR2 numerology
        if (numSubcarriers == 0) numSubcarriers = 3168; // 264 RBs (400 MHz bandwidth)
     } else {
        NS_FATAL_ERROR("This executable is urban-micro only.");
    }
    ...
    else
    {
        ...
        Ptr<PropagationLossModel> ns3Loss =
        ...
        // Use the 3GPP probabilistic UMi channel-condition model (TR 38.901
        // sec 7.4.2) so ns3 mode produces a realistic LOS/NLOS mix from
        // geometry, matching what Sionna ray tracing derives from the scene.
        // Shadowing is left at its default (enabled, TR 38.901 sigma values)
        // so per-UE link-budget variation matches Sionna's blockage-driven
        // fading. The only intentional difference between the two simModes is
        // now the propagation engine itself (3GPP stochastic vs. ray traced).
        auto condModel = CreateObject<ThreeGppUmiStreetCanyonChannelConditionModel>();
    ...
    apGnb.downtiltAngle = 6.0 * M_PI / 180.0;   // UMi street-canyon, 10 m mast
