...
main(int argc, char* argv[]) {
    ...
    std::string scenario = "free_space";
    ...
    uint32_t seed = 42;
    // FR1 low-band macro defaults: 46 dBm BS conducted power, 23 dBm UE PCmax.
    // Free-space deployment is link-budget-friendly (no clutter), so the goal
    // here is to test long range rather than dense capacity.
    ...
    uint32_t numGnb = 1;
    uint32_t numUes = 30;
    // Free-space defaults: long-reach n1 (2 GHz) macro deployment. Friis
    // path loss at 2 GHz with 46 dBm + 8x8 beamforming covers well past 1 km;
    // even a 2x2 array reaches ~500 m. Pick a wide annulus to exercise both
    // near and far link-budget regimes in a single sweep.
    double ueDist_min = 50.0;
    double ueDist_max = 1000.0;
    // Omnidirectional cell (single-cell free-space scenario): place UEs all
    // around the gNB. Real free-space rural cells are commonly omni or 3-sector
    // with very wide HPBW; pick omni so UEs sample the full antenna pattern.
    double ueAzimuthMinDeg = 0.0;
    double ueAzimuthMaxDeg = 360.0;
    ...
    cmd.AddValue("scenario", "Fixed scene for this executable: free_space", scenario);
    ...
    if (all)
    {
        ...
        std::cout << "Running all free-space simulations: " << results.capacity()
        ...
        const std::filesystem::path summaryPath =
            std::filesystem::path(outputDir) / "free_space_all_runs_summary.csv";
    ...
    if (scenario != "free_space") {
        NS_FATAL_ERROR("This executable is free-space only. Use --scenario=free_space or omit the scenario argument.");
    }
    ...
    if (scenario == "free_space")
    {
        ...
        gnbHeight = 35.0;     // Free-space rural/macro mast height
        if (ueDist_min <= 0.0) ueDist_min = 50.0;
        if (ueDist_max <= 0.0) ueDist_max = 1000.0;  // Long-reach annulus for FSPL test
    }
    else
    {
        NS_FATAL_ERROR("This executable is free-space only.");
    }
    ...
    // -----------------------------------------------------------------------
    if (scenario == "free_space") {
        // FR1 Band n1 -- 2 GHz low-band macro, used by long-reach rural and
        // satellite-style deployments. Wavelength is large (~15 cm) so the
        // 8x8 array footprint is bigger than at FR2 but coverage is excellent.
        if (f_c == 0.0) f_c = 2.0e9;             // n1 carrier
        if (scs == 0) scs = 15000;                // 15 kHz numerology mu=0
        if (numSubcarriers == 0) numSubcarriers = 1200; // 100 RBs (20 MHz bandwidth)
     } else {
        NS_FATAL_ERROR("This executable is free-space only.");
    }
    ...
    {
        // Free-space ns3 mode: pure Friis path loss (no clutter, no shadowing,
        // always-LOS by definition). The 3GPP RMa scenario is the closest
        // small-scale-fading model in the spectrum loss helper -- it captures
        // sparse-multipath rural channels which is the right complement to
        // FSPL. The only intentional difference between sionna and ns3 modes
        // for free-space is "ray-traced" vs "Friis + 3GPP RMa fading".
        ...
        auto condModel = CreateObject<AlwaysLosChannelConditionModel>();
        ...
        spectrumLoss->SetChannelModelAttribute("Scenario", StringValue("RMa"));
    ...
    apGnb.downtiltAngle = 3.0 * M_PI / 180.0;   // free-space, 35 m mast, far-reach
