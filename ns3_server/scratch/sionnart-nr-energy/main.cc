...
NS_LOG_COMPONENT_DEFINE("SionnartNrEnergy");
...
main(int argc, char* argv[]) {
    ...
    uint16_t gnbAntennaRows = 4;
    uint16_t gnbAntennaCols = 4;
    ...
    uint32_t numGnb = 1;
    uint32_t numUes = 30;
    double ueDist_min = 0.0;
    double ueDist_max = 0.0;
    double ueAzimuthMinDeg = 80.0;
    double ueAzimuthMaxDeg = 100.0;
    ...
    cmd.AddValue("scenario", "Scene: `free_space` or `urban_micro` or `urban_macro`", scenario);
    ...
    if (scenario != "free_space" && scenario != "urban_micro" && scenario != "urban_macro") {
        NS_FATAL_ERROR("Invalid scenario: " << scenario << ". Use 'free_space', 'urban_micro' or 'urban_macro'.");
    }
    ...
    if (scenario == "urban_macro")
    {
        ...
        gnbHeight = 25.0;     // 3GPP UMa standard
        if (ueDist_min <= 0.0) ueDist_min = 35.0;
        if (ueDist_max <= 0.0) ueDist_max = 500.0;   // UMa cell radius
    }
    else if (scenario == "urban_micro")
    {
        ...
        if (ueDist_max <= 0.0) ueDist_max = 200.0;   // UMi cell radius
    }
    else
    {
    ...
    if (scenario == "urban_macro") {
        ...
        if (f_c == 0.0) f_c = 28.0e9; // FR2 mmWave
        if (scs == 0) scs = 120000; // 120kHz
        if (numSubcarriers == 0) numSubcarriers = 3168; // 264 RBs (400 MHz bandwidth)
    } else if (scenario == "free_space") {
        if (f_c == 0.0) f_c = 3.5e9; // FR1 n78
        if (scs == 0) scs = 30000; // 30kHz
        if (numSubcarriers == 0) numSubcarriers = 3276; // 273 RBs (100MHz bandwidth)
    } else {
        NS_LOG_ERROR("Unknown scenario: " << scenario);
        return 1;
    }
    ...
    else
    {
        ...
        ns3Loss->SetAttributeFailSafe("ShadowingEnabled", BooleanValue(false));
