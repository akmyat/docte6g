...
NS_LOG_COMPONENT_DEFINE("WarehouseIsacDemo");
...
int main(int argc, char *argv[])
{
  ...
  std::string outputDir = "/home/aung/code/ns3_sionna_server/results/warehouse_isac/";
  uint32_t seed = 1;
  ...
  // WH_WAYPOINTS from isac.ipynb (x,y only; z=1.5 added below)
  std::vector<std::vector<std::array<double,2>>> robotWaypoints = {
      {{11.5,9.0},{11.5,-8.0},{-12.0,-8.0},{-12.0,-7.0}},  // robot_1
      {{11.5,4.0},{11.5,-8.0},{ -7.0,-8.0},{ -7.0,-7.0}},  // robot_2
      {{11.5,-1.0},{ 2.0,-1.0},{ -2.0,-1.0},{ -2.0,-3.0}}, // robot_3
  };
  ...
  sionnaSettings.rx_type_path          = basePath + "objects/iw_hub/iw_hub.ply";
  ...
  for (int i = 0; i < 3; ++i) {
      ...
      // Robot-mounted camera MQTT + app (shares the node, separate MQTT client)
  ...
  // =========================================================================
  {
      ...
      os << "--- Simulation Configuration (ISAC) ---\n"
  ...
  // ISAC: export radar-detected object positions vs. actual robot positions
  {
      std::ofstream os(outputDir + "/isac_detected_objects.csv");
      os << "Time_s,TrackId,X,Y,Z,Actual_X,Actual_Y,Actual_Z,Error_X,Error_Y,Error_Z,Error_Dist\n";
      auto records = SionnaPyEmbed::GetInstance().SionnaGetDetectedObjects();

      auto GetInterpolatedPosition = [&](uint32_t nodeId, double t) -> Vector {
          double before_t = -1.0, after_t = 1e9;
          Vector before_pos(0,0,0), after_pos(0,0,0);
          bool foundBefore = false, foundAfter = false;
          for (const auto& md : g_sionnaMobilityData) {
              if (md.nodeId == nodeId) {
                  double md_t = md.timestamp.GetSeconds();
                  if (md_t <= t && md_t >= before_t) { before_t = md_t; before_pos = md.position; foundBefore = true; }
                  if (md_t >= t && md_t <= after_t)  { after_t  = md_t; after_pos  = md.position; foundAfter  = true; }
              }
          }
          if (!foundBefore && foundAfter)  return after_pos;
          if (foundBefore  && !foundAfter) return before_pos;
          if (!foundBefore && !foundAfter) return Vector(0,0,0);
          if (after_t == before_t) return before_pos;
          double ratio = (t - before_t) / (after_t - before_t);
          return Vector(
              before_pos.x + ratio * (after_pos.x - before_pos.x),
              before_pos.y + ratio * (after_pos.y - before_pos.y),
              before_pos.z + ratio * (after_pos.z - before_pos.z));
      };

      for (const auto& r : records) {
          double minError = 999999.0;
          Vector bestActual(0,0,0);
          Vector detectedPos(r.x, r.y, r.z);
          // Robot nodes are UE indices 8-10
          for (uint32_t idx = 8; idx <= 10; ++idx) {
              uint32_t nid = ueNodes.Get(idx)->GetId();
              Vector act = GetInterpolatedPosition(nid, r.time);
              double dx = act.x - detectedPos.x;
              double dy = act.y - detectedPos.y;
              double dz = act.z - detectedPos.z;
              double err = std::sqrt(dx*dx + dy*dy + dz*dz);
              if (err < minError) { minError = err; bestActual = act; }
          }
          os << r.time << "," << r.track_id << ","
             << r.x << "," << r.y << "," << r.z << ","
             << bestActual.x << "," << bestActual.y << "," << bestActual.z << ","
             << (detectedPos.x - bestActual.x) << ","
             << (detectedPos.y - bestActual.y) << ","
             << (detectedPos.z - bestActual.z) << ","
             << minError << "\n";
      }
  }
  ...
  std::cout << "Simulation complete (ISAC). Results saved to " << outputDir << std::endl;
